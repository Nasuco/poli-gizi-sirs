...
<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3 " id="sidenav-main">
  <div class="sidenav-header">
    ...
    <a class="align-items-center d-flex m-0 navbar-brand text-wrap" href="">
        <img src="../assets/img/logo-pku.png" class="navbar-brand-img h-100" alt="...">
        <span class="ms-3 font-weight-bold">Sistem Informasi Rumah Sakit</span>
    </a>
  </div>
  ...
  <div class="w-auto" id="sidenav-collapse-main">
    <ul class="navbar-nav">
      <li class="nav-item">
        {{-- <a class="nav-link {{ (Request::is('kitchen/home') ? 'active' : '') }}" href="{{ url('kitchen/home') }}"> --}}
          {{-- @if(auth()->user()->type === 'superadmin')
              <a class="nav-link {{ (Request::is('superadmin/home') ? 'active' : '') }}" href="{{ url('/home') }}"> --}}
          @if(auth()->user()->type === 'ahligizi')
                  <a class="nav-link {{ (Request::is('ahligizi/home') ? 'active' : '') }}" href="{{ url('ahligizi/home') }}">
          @elseif(auth()->user()->type === 'kitchen')
                  <a class="nav-link {{ (Request::is('kitchen/home') ? 'active' : '') }}" href="{{ url('kitchen/home') }}">
          @elseif(auth()->user()->type === 'distributor')
                  <a class="nav-link {{ (Request::is('distributor/home') ? 'active' : '') }}" href="{{ url('distributor/home') }}">
      ...
      <!-- <li class="nav-item pb-2">
        <a class="nav-link {{ (Request::is('screening') ? 'active' : '') }}" href="{{ url('screening') }}">
            <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                <i style="font-size: 1rem;" class="fas fa-lg fa-list-ul ps-2 pe-2 text-center text-dark {{ (Request::is('screening') ? 'text-white' : 'text-dark') }} " aria-hidden="true"></i>
            </div>
            <span class="nav-link-text ms-1">Screening</span>
        </a>
      </li> -->
      <li class="nav-item mt-2">
        <h6 class="ps-4 ms-2 text-uppercase text-xs font-weight-bolder opacity-6">Halaman Tabel</h6>
      </li>
      <li class="nav-item">
        @if(auth()->user()->type === 'ahligizi')
        <a class="nav-link {{ (Request::is('pasien') ? 'active' : '') }}" href="{{ url('pasien') }}">
          ...
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-person-fill" viewBox="0 0 16 16">
            <path class="color-background opacity-6" d="M3 14s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1zm5-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6"/>
          </svg>
          </div>
          <span class="nav-link-text ms-1">Pasien</span>
        </a>
        <!-- <a class="nav-link {{ (Request::is('screening/create') ? 'active' : '') }}" href="{{ url('screening/create') }}">
          <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
          ...
          <span class="nav-link-text ms-1">Buat Skrining</span>
        </a> -->
      </li>
      <li class="nav-item">
        @elseif(auth()->user()->type === 'kitchen')
        <a class="nav-link {{ (Request::is('kitchen/menumakan') ? 'active' : '') }}" href="{{ url('kitchen/menumakan') }}">
          <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
          ...
          <span class="nav-link-text ms-1">Menu Makan Pasien</span>
        </a>
      </li>
      <li class="nav-item">
        @elseif(auth()->user()->type === 'distributor')
        
        <a class="nav-link {{ (Request::is('distributor/makananproses') ? 'active' : '') }}" href="{{ url('distributor/makananproses') }}">
          <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
          ...
          <span class="nav-link-text ms-1">Pengantaran Proses</span>
        </a>

        <a class="nav-link {{ (Request::is('distributor/makananselesai') ? 'active' : '') }}" href="{{ url('distributor/makananselesai') }}">
          <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
            <svg width="12px" height="12px" viewBox="0 0 42 42" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
          ...
          <span class="nav-link-text ms-1">Pengantaran Selesai</span>
        </a>
      
        <a class="nav-link {{ (Request::is('pengantaran') ? 'active' : '') }}" href="{{ url('pengantaran') }}">
          <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
          <svg class="text-dark" width="16px" height="16px" viewBox="0 0 40 40" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"> <title>settings</title> <g id="Basic-Elements" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"> <g id="Rounded-Icons" transform="translate(-2020.000000, -442.000000)" fill="#FFFFFF" fill-rule="nonzero"> <g id="Icons-with-opacity" transform="translate(1716.000000, 291.000000)"> <g id="settings" transform="translate(304.000000, 151.000000)"> <polygon class="color-background" id="Path" opacity="0.596981957" points="18.0883333 15.7316667 11.1783333 8.82166667 13.3333333 6.66666667 6.66666667 0 0 6.66666667 6.66666667 13.3333333 8.82166667 11.1783333 15.315 17.6716667"></polygon> <path class="color-background" d="M31.5666667,23.2333333 C31.0516667,23.2933333 30.53,23.3333333 30,23.3333333 C29.4916667,23.3333333 28.9866667,23.3033333 28.48,23.245 L22.4116667,30.7433333 L29.9416667,38.2733333 C32.2433333,40.575 35.9733333,40.575 38.275,38.2733333 L38.275,38.2733333 C40.5766667,35.9716667 40.5766667,32.2416667 38.275,29.94 L31.5666667,23.2333333 Z" id="Path" opacity="0.596981957"></path> <path class="color-background" d="M33.785,11.285 L28.715,6.215 L34.0616667,0.868333333 C32.82,0.315 31.4483333,0 30,0 C24.4766667,0 20,4.47666667 20,10 C20,10.99 20.1483333,11.9433333 20.4166667,12.8466667 L2.435,27.3966667 C0.95,28.7083333 0.0633333333,30.595 0.00333333333,32.5733333 C-0.0583333333,34.5533333 0.71,36.4916667 2.11,37.89 C3.47,39.2516667 5.27833333,40 7.20166667,40 C9.26666667,40 11.2366667,39.1133333 12.6033333,37.565 L27.1533333,19.5833333 C28.0566667,19.8516667 29.01,20 30,20 C35.5233333,20 40,15.5233333 40,10 C40,8.55166667 39.685,7.18 39.1316667,5.93666667 L33.785,11.285 Z" id="Path"></path></g> </g> </g> </g> </svg>
            ...
            <span class="nav-link-text ms-1">Daftar Paisen</span>
          </a>
      </li>
      <li class="nav-item">
        @elseif(auth()->user()->type === 'distributor')
        <a class="nav-link {{ (Request::is('distributor/tables') ? 'active' : '') }}" href="{{ url('distributor/tables') }}">
          <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
            <svg width="12px" height="12px" viewBox="0 0 42 42" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
              <title>box-3d-50</title>
              <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
      ...
      <li class="nav-item pb-2">
        <a class="nav-link {{ (Request::is('screening') ? 'active' : '') }}" href="{{ url('screening') }}">
            ...
            <span class="nav-link-text ms-1">Skrining</span>
        </a>
      </li>
